...
import {AuthDispatchType, ActionNames, AuthAction, AuthResponseData, CreateSyncDispatch, LocalStorageKeys} from './types';
...
export const authSuccess = (token: string, userId: string): AuthAction<ActionNames.AUTH_SUCCESS> => (
    {
...
export const auth = (email: string, password: string, isSignUp: boolean) => {
    return async (dispatch: CreateSyncDispatch<ActionNames.AUTH_SUCCESS | ActionNames.AUTH_FAIL> & AuthDispatchType<ActionNames.LOGOUT>) => {
        try {
            const authData = {
                email,
                password,
            ...
            let url = 'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDBasCUXhdhZTn4zf9rVSs0bHbmiCeskHw';
            if (!isSignUp) {
                url = 'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDBasCUXhdhZTn4zf9rVSs0bHbmiCeskHw';
            }
            ...
            const expirationDate = new Date(currentDate.getTime() + parseInt(response.data.expiresIn)*1000).getTime();
            ...
            if (err.response.data.error.message && typeof err.response.data.error.message === 'string') {
                ...
                dispatch(authFail('Smth wrong with code'))
            }
        }
    }
}

export const setAuthRedirectPath = (path: string): AuthAction<ActionNames.SET_AUTH_REDIRECT_PATH> => ({
...
export const authCheckState = () => {
    return (dispatch: CreateSyncDispatch<ActionNames.LOGOUT | ActionNames.AUTH_SUCCESS> & AuthDispatchType<ActionNames.LOGOUT>) => {
