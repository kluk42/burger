...
export const purchaseBurgerSuccess = (id: string, order: Order): OrderAction<ActionNames.PURCHASE_BURGER_SUCCESS> => {
    return{
...
export const purchaseBurger = (order: Order, token: string) => {
    return async (dispatch: DispatchOrderAction<
        ...
        >) =>
    {
        ...
        try {
            const response: AxiosResponse<{name: string}> = await axios.post('/orders.json?auth=' + token, order);
...
export const fetchOrdersSuccess = (orders: OrderFromServer[]): OrderAction<ActionNames.FETCH_ORDERS_SUCCESS> => {
    return {
...
export const fetchOrders = (token: string, userId: string) => {
    return async (dispatch: DispatchOrderAction<ActionNames.FETCH_ORDERS_INIT | ActionNames.FETCH_ORDERS_SUCCESS | ActionNames.FETCH_ORDERS_FAIL>) => {
